            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="flex items-center justify-between gap-4 p-4">
                    <div className="text-sm text-gray-600">
                        ...
                        <span className="text-gray-500">Showing {(banners.length === 0) ? 0 : (currentPage - 1) * PAGE_SIZE + 1} - {Math.min(currentPage * PAGE_SIZE, banners.length)}</span>
                    </div>

                    <div className="flex items-center gap-3">
                        <label className="text-sm text-gray-600">Sort:</label>
                        <select value={sortOption} onChange={(e) => { setSortOption(e.target.value as any); setCurrentPage(1); }} className="px-3 py-1 text-sm border rounded-md bg-white">
                ...
                {banners.length > PAGE_SIZE && (
                    <div className="flex items-center justify-between gap-4 p-4 border-t border-gray-100">
                        <div className="text-sm text-gray-600">Page {currentPage} of {Math.max(1, Math.ceil(banners.length / PAGE_SIZE))}</div>
